...
def slot(start_date, clock, zone, rule, index):
    day = occurrence_date(date.fromisoformat(start_date), rule, index)
    return local_timestamp(day, clock, zone) if day else None


def next_slot(start_date, clock, zone, rule, after, minimum_index=0):
    ...
    while low < high:
        mid = (low + high) // 2
        value = slot(start_date, clock, zone, rule, mid)
        if value is None or value >= after:
            high = mid
        else:
            low = mid + 1
    value = slot(start_date, clock, zone, rule, low)
