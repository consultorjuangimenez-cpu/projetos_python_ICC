...
from .validation import collaborator_input, search_key
...
class Store:
        ...
        with self.db(True) as con:
            ...
            if task_id is None:
                ...
                if old['status'] == 'CANCELADO':
                    raise ValueError('Tarefa cancelada não pode ser editada. Cadastre uma nova tarefa.')
                con.execute("UPDATE deliveries SET state='CANCELED',finished_at=? WHERE task_id=? AND state IN ('WAITING','ERROR')", (now, task_id))
                status = old['status']
    ...
    def change_status(self, task_id, action, actor, revision, now):
        with self.db(True) as con:
            ...
            skipped = 0
            if action == 'pause' and status == 'ATIVO':
                status = 'PAUSADO'
            ...
            con.execute("UPDATE deliveries SET state='CANCELED',finished_at=? WHERE task_id=? AND state IN ('WAITING','ERROR')", (now, task_id))
            con.execute('UPDATE tasks SET status=?,next_ts=?,next_index=?,revision=revision+1,updated_at=?,last_error=NULL,skipped_count=skipped_count+? WHERE id=?',
                        (status, next_ts, idx, now, skipped, task_id))
            self._audit(con, task_id, actor, action.upper(), f'{skipped} ocorrência(s) transcorrida(s) durante a pausa.' if skipped else '', now=now)

    @staticmethod
    def _enqueue(con, key, task, kind, actor, now, payload=None):
        ...
        con.execute("""INSERT OR IGNORE INTO deliveries(request_key,task_id,kind,generation,ordinal,scheduled_ts,payload,
            actor_id,actor_name,state,available_at,created_at,message_id) VALUES(?,?,?,?,?,?,?,?,?,'WAITING',?,?,?)""",
            (key, task['id'] if task else None, kind, task['revision'] if task else None,
             ...
             actor['id'], actor['name'], now, now, message_id))
    ...
    def queue_manual(self, task_id, actor, key, now, revision):
        with self.db(True) as con:
            ...
            if task['status'] == 'CANCELADO':
                raise ValueError('Tarefa cancelada não aceita novos envios.')
            if con.execute("SELECT 1 FROM deliveries WHERE task_id=? AND kind='MANUAL' AND state='WAITING'", (task_id,)).fetchone():
    ...
    def queue_due(self, now):
        with self.db(True) as con:
            rows = con.execute("""SELECT * FROM tasks t WHERE status='ATIVO' AND next_ts<=?
    ...
    def claim(self, now):
        with self.db(True) as con:
            row = con.execute("SELECT * FROM deliveries WHERE state='WAITING' AND available_at<=? ORDER BY available_at,id LIMIT 1", (now,)).fetchone()
            if not row:
                return None
            delivery = unpack(row)
            if delivery['task_id']:
                task = con.execute('SELECT * FROM tasks WHERE id=?', (delivery['task_id'],)).fetchone()
                valid = task and task['status'] != 'CANCELADO' and task['revision'] == delivery['generation']
        ...
        with self.db(True) as con:
            ...
            if action == 'retry' and delivery['state'] == 'ERROR':
                if delivery['task_id']:
                    ...
                    if task['revision'] != delivery['generation'] or task['status'] == 'CANCELADO' or (delivery['kind'] == 'SCHEDULED' and task['status'] != 'ATIVO'):
    ...
    def dashboard(self, actor, now, zone):
        ...
        with self.db() as con:
            ...
            result['sent'] = con.execute("SELECT COUNT(*) FROM deliveries d JOIN tasks t ON t.id=d.task_id WHERE d.state='SENT'"+('' if actor.get('admin') else ' AND t.created_by=?'), params).fetchone()[0]
            return result
    ...
    def history(self, actor, task_id=None, page=1, result=None):
        ...
        with self.db() as con:
            ...
            rows = con.execute('SELECT h.*,d.task_id,d.kind,d.payload,d.state,d.actor_name'+base+where+' ORDER BY h.id DESC LIMIT 50 OFFSET ?', (*params,(page-1)*50)).fetchall()
            return [unpack(row) for row in rows], total
