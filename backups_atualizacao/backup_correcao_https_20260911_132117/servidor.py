"""Portal interno: menu e aplicação PDF no mesmo endereço HTTP."""
import logging
import socket
import threading
import time
...
def open_when_ready():
    # Só este processo abre o navegador, uma vez, depois da resposta HTTP.
    for _ in range(30):
        try:
...
def main():
    ...
    try:
        server = create_server(application, host='0.0.0.0', port=PORT, threads=6,
