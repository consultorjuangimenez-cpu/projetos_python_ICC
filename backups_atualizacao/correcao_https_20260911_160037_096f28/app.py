...
def create_app(base=BASE, inventory=None):
    ...
    def ready_https():
        ...
        return (via_https_proxy() and request.is_secure and url.scheme=='https' and bool(url.hostname)
    ...
    def bridge_tls():
        url=urlsplit(cfg['url'])
        if (not via_https_proxy() or not request.is_secure or url.scheme!='https'
                or request.host.lower()!=url.netloc.lower()):
            abort(403,description='A instalação exige HTTPS pelo proxy configurado pela TI.')
