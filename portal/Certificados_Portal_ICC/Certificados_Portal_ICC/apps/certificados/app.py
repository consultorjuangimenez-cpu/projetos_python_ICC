...
def create_app(base=BASE, inventory=None):
    ...
    def ready_https():
        ...
        return (request.is_secure and url.scheme=='https' and bool(url.hostname)
                and not url.username and not url.password and not url.query and not url.fragment
                and url.netloc.lower()==request.host.lower()
                and url.path.rstrip('/')==request.script_root.rstrip('/'))

    def bridge_tls():
        url=urlsplit(cfg['url'])
        if not request.is_secure or url.scheme!='https' or request.host.lower()!=url.netloc.lower():
            abort(403,description='A instalação exige HTTPS no endereço configurado pela TI.')
    ...
    def password(ident):
        if not request.is_secure: abort(403,description='Use HTTPS para enviar a senha corrigida.')
    ...
    app.wsgi_app=DispatcherMiddleware(app.wsgi_app,{'/agente-api':bridge})
    return app
